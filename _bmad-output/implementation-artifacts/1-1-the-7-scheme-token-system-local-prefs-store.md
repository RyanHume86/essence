...
- [x] **Task 2 — Build the `usePrefs` local store (AC: 1, 4, 6)**
  - [x] Create `src/lib/prefs.js` — **pure, tested** logic: `DEFAULT_PREFS`, `SCHEMES` catalog (key, label, mode), `readPrefs()` / `writePrefs(patch)` over `localStorage` key `akha_prefs`, and `normalizePrefs(raw)` (coerce unknown/missing `scheme` → `deep-sea`, fill defaults). No React, no Base44 import.
...
- **Storage:** localStorage key `akha_prefs`, JSON `{ name, scheme, creature, onboarded }`. Follows the shipped `akha_*` key convention (`akha_win_*`, `akha_calendar_connected`). **Never** written to Base44 — no cross-device pref sync in v1 (a future cloud mirror would live *behind* `usePrefs`). [AD-6, Deferred]
...
- [x] [Review][Patch] Scheme FOUC on cold load for non-default schemes [index.html] — **FIXED**: added a synchronous pre-paint `<script>` in `index.html` that reads `akha_prefs` and sets `data-scheme` before first paint (usePrefs reconciles/validates on mount).
