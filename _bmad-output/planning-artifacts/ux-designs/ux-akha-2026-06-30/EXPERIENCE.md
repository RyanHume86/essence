---
name: Akha
description: Information architecture, behavior, states, interactions, and journeys for Akha.
status: final
created: 2026-06-30
updated: 2026-06-30
...
1. **Arrival.** Ryan opens Akha, tired, not eager. The creature greets him with a soft smile and a brief encouraging welcome — it does not pounce (FR18).
...
4. **Decompose.** He breaks tasks into subtasks; Akha gently nudges decomposition up front via a **single passive affordance** (placeholder / hint text only) — never a recurring prompt, never a warning, never blocking save — and subtasks stay editable later (FR6/FR7).
...
1. Ryan installs Akha and opens it with zero tasks.
...
- **No independence-takeover.** No prescribed tasks, no preloaded or default lists, no deciding the day for the user. The user plans; Akha supports (rule 4).
